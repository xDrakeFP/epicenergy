...
nomeUtente.innerHTML = `<p style="font-size:1.3rem" > Ciao, <span style= font-weight:bolder; >${localStorage.getItem("username")}</span> </p>`;
...
async function loadClients() {
    try {
        const token = localStorage.getItem("token");
        const response = await fetch(`${API_BASE_URL}/client?page=${currentPage}&size=${pageSize}`, {
            headers: {
...
function displayClients(clienti) {
    ...
    <tr>
      ...
      <td>€${cliente.fatturatoAnnuale ? cliente.fatturatoAnnuale.toLocaleString() : "0"}</td>
    </tr>
  `
        )
        .join("");
